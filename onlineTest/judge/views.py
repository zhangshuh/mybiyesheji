...
def add_tiankong(request):
    if request.method == 'POST':  # 当提交表单时
        ...
        if form.is_valid():  # 如果提交的数据合法
            ...
            return redirect(reverse("tiankongproblem_detail", args=[problem.problem_id]))
    else:  # 当正常访问时
...
class GaicuoProblemDetailView(DetailView):
    model = GaicuoProblem
    template_name = 'gaicuo_detail.html'
    context_object_name = 'problem'
...
class TiankongProblemDetailView(DetailView):
        ...
        template_name = 'tiankongproblem_detail.html'
        context_object_name = 'problem'
    ...
    if request.method == "POST":  # 当提交表单时
        ...
        if form.is_valid():
            ...
            try:  # 对文件进行解压和保存
                ...
                print(e)
                pass
            return redirect(reverse("tiankongproblem_detail", args=[id]))
...
def list_tiankong(request):
    classnames = ClassName.objects.all()
    return render(request, 'tiankong_problem_list.html', context={'classnames': classnames, 'title': '程序填空题题库'})

# 程序gaicuo题列表
def list_gaicuo(request):
    classnames = ClassName.objects.all()
    return render(request, 'gaicuo_problem_list.html', context={'classnames': classnames, 'title': '程序改错题题库'})
